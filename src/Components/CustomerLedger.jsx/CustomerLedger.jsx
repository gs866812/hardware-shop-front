...
const CustomerLedger = () => {
  ...
  useEffect(()=> {
    axiosProtect.get(`/customerLedger`, {
      params: {
        userEmail: mail,
        page: currentPage,
        size: itemsPerPage,
  ...
  const handlePageClick = (page) => {
    ...
    return (
      ...
      <div className="flex items-center justify-between">
        <h2 className="text-2xl">Customer ledger</h2>
                ...
                {Array.isArray(customer) &&
                  customer.map((customer) => (
                    <tr key={customer._id}>
                      ...
                      <td> <IoEyeOutline onClick={()=> handleCustomerLedger(customer.customerSerial)} className="text-xl cursor-pointer hover:text-yellow-500"/></td>
                    </tr>
                  ))}
              </tbody>
            </table>
